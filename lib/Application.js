...
class Application
{
    ...
    async _masterReceive(worker, msg)
    {
        try
        {
            ...
            if(service._db) this.releaseDbConnection(service._db);
        }
        catch (err)
        {
            msg.error = err;
    ...
    async loadModels(name, path)
    {
        ...
        for(let i in files)
        {
            ...
            {
                let obj = new(classs)(this, name);
                obj.createConnection = async function ()
                {
                    if(!this._db)
                    {
                        this._db = await this.app.getDbConnection();
                    }

                    return this._db;
                }

                this.services[name].model[temp] = obj;
            }
        ...
        glob(path + "/view/*", function (er, files)
        {
            for(var i in files)
            {
                ...
                else if(ext == 'ejs')
                {
                    ...
                    try
                    {
                        let compiled = ejs.compile(loaded);
                        self.services[name].view[temp] = [ext, compiled];
                    }
                    catch( err )
                    {
                        err.message = "`" + name + "." + temp + "View` " + err.message;
                        console.error( err );
                    }
                }
                else if(ext == 'js')
    ...
    prepareUri(ctx)
    {
        ...
        {
            let service = this.defaultService;
        ...
        if(ctx.uri.controller == process.env.CMS_PATH)
            {
                ...
                target = this.services['mvc-base'].controller[name];
    ...
    async run(ctx)
    {
        try
        {
            ...
            // Look for the app
            target = this.services[ctx.uri.service].controller[name];

            // console.log( this.services );
            // Look for base app
            if(target == null) target = this.services['mvc-base'].controller[name];

            // Look for home (if it has method for it or accepts any )
            if(target == null)
            {
                target = this.services[ctx.uri.service].controller.Home;

                if(target == null)
                {
                    target = this.run404(ctx);
                }
                else if(target[name.firstCharToLowerCase()] != null)
                {
                    if( ctx.uri.method != 'index' ) ctx.uri.vars.unshift(ctx.uri.method);
                    ctx.uri.method = ctx.uri.controller;
                    ctx.uri.controller = 'home';
                }
                else if(target.any != null)
                {
                    ctx.uri.vars.unshift(ctx.uri.controller, ctx.uri.method);
                    ctx.uri.method = 'any';
                    ctx.uri.controller = 'home';
                }
                else
    ...
    run404(ctx)
    {
        ...
        var target = this.services[this.defaultService].controller.Error;

        // Look for base app
        if(target == null) target = this.services['mvc-base'].controller.Error;
    ...
    {
        // Look for the app
        let target = this.services[ctx.uri.service].controller.Error;

        // Look for default app
        if(target == null) target = this.services[this.defaultService].controller.Error;
     ...
     */
    getModel(name, service = this.defaultService)
    {
        // Look for the app
        ...
        // Look for base app
        if(target == null) target = this.services['mvc-base'].model[name];
     ...
     * @param {string} service - The service`s name.
     */
    getView(name, service = this.defaultService)
    {
        // Look for default app
        var target = this.services[service].view[name];

        // Look for base app
        if(target == null) target = this.services['mvc-base'].view[name];
    ...
    /**
     * Add custom objects for views to be rendered.
     * @access protected
     * @param {Object} vars - Variables for the view render.
     */
    setGlobalVarsToRenderViews(vars)
    {
        return vars;
    }
     ...
     * @access protected
     * @param {Object} conn - A database connection to be released.
     */
