...
module.exports = class Application
{
    init(server)
    {
        ...
        this.setDefaultApp('base', __dirname + "/../" + 'base');
    }
    ...
    async loadModels(name, path)
    {
        ...
        {
            let file = files[i];
            let temp = file.split('/').pop().split('Model.js')[0].toLowerCase();
            let classs = require.main.require(file);

            if(this.isMaster && classs.instanceMode() == BaseModel.MASTER_CLUSTER ||
                !this.isMaster && classs.instanceMode() == BaseModel.SLAVE_CLUSTER)
            {
                this.apps[name].model[temp] = new(classs)(this);
            }
            else if(!this.isMaster && classs.instanceMode() == BaseModel.MASTER_CLUSTER)
            {
                ...
                this.apps[name].model[temp] = service;
            }
        }
    }
